...
export default function SimpleVideoForm() {
  ...
  return (
    <form action={insertVideoAction} className="space-y-4">
      ...
      {/* cover_image for mp4: auto-filled from first-frame extraction */}
      {sourceType === "mp4" && (
        <input type="hidden" name="cover_image" value={autoCoverUrl} />
      )}
            ...
            {autoCoverUrl && (
              <div className="flex items-center gap-3 p-3 rounded-xl bg-emerald-50/60 border border-emerald-100">
                <img
                  src={autoCoverUrl}
                  alt="封面预览"
                  className="w-10 h-[71px] object-cover rounded-lg shrink-0"
                />
                <p className="text-[11px] text-emerald-700 font-medium">✓ 封面已从首帧自动生成</p>
              </div>
            )}
          </>
        )}
